...
if __name__ == "__main__":
    ...
    for classification in df['Classificació'].unique():
        ...
        stats_with_stopwords[classification]['mean_len_words'] = max(df['Description'].str.split().apply(lambda x: [len(i) for i in x]).map(lambda x: np.mean(x)).hist())
    ...
    for classification in df['Classificació'].unique():
        ...
        stats_without_stopwords[classification]['mean_len_words'] = max(df['Description'].str.split().apply(lambda x: [len(i) for i in x]).map(lambda x: np.mean(x)).hist())
    ...
    with open(r'res/stats_with_stopwords.csv', 'w', encoding="utf-8") as w_file:
    ...
    with open(r'res/stats_without_stopwords.csv', 'w', encoding="utf-8") as w_file:
        w_file.write('class,mean_len_txts,mean_len_words,1_grama,2_grama,3_grama')
